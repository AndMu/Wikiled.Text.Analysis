...
{
    /// <summary>
    /// Regexs and regular expressions used by the twitter text methods.
    /// </summary>
    internal static class TwitterRegex
    {
        ...
        // to access both the list of characters and a Regex suitible for use with String#split
        ...
        private static readonly string VALID_URL_Regex_STRING =
            "(" +                                                   //  $1 total match
        ...
        // Cashtag related Regexs
        private const string CASHTAG = "(?:[a-z]{1,2}[:._-]{1}[0-9]{1,6})|(?:[a-z]{1,6}(?:[:._-]{1}(?:[a-z]|[0-9]){1,6})?)|(?:[0-9]{1,6}[:._-]{1}[a-z]{1,2})";

        //
        // Begin internal constants
        //
        internal static readonly Regex INVALID_CHARACTERS = new Regex(INVALID_CONTROL_CHARS, RegexOptions.IgnoreCase);

        internal static readonly Regex VALID_HASHTAG = new Regex(VALID_HASHTAG_STRING, RegexOptions.IgnoreCase);
        ...
        internal static readonly Regex INVALID_HASHTAG_MATCH_END = new Regex("^(?:[#＃]|://)");

        internal static readonly Regex RTL_CHARACTERS = new Regex("[" + RTL_CHARS + "]");

        internal static readonly Regex AT_SIGNS = new Regex("[" + AT_SIGNS_CHARS + "]");

        internal static readonly Regex VALID_MENTION_OR_LIST = new Regex("([^a-z0-9_!#$%&*" + AT_SIGNS_CHARS + "]|^|(?:^|[^a-z0-9_+~.-])RT:?)(" + AT_SIGNS + "+)([a-z0-9_]{1,20})(/[a-z][a-z0-9_\\-]{0,24})?", RegexOptions.IgnoreCase);
        ...
        internal static readonly Regex VALID_REPLY = new Regex("^(?:" + UNICODE_SPACES + ")*" + AT_SIGNS + "([a-z0-9_]{1,20})", RegexOptions.IgnoreCase);
        internal const int VALID_REPLY_GROUP_USERNAME = 1;

        internal static readonly Regex INVALID_MENTION_MATCH_END = new Regex("^(?:[" + AT_SIGNS_CHARS + LATIN_ACCENTS_CHARS + "]|://)");

        internal static readonly Regex VALID_URL = new Regex(VALID_URL_Regex_STRING, RegexOptions.IgnoreCase);
        ...
        internal static readonly Regex VALID_TCO_URL = new Regex("^https?:\\/\\/t\\.co\\/[a-z0-9]+", RegexOptions.IgnoreCase);

        internal static readonly Regex INVALID_URL_WITHOUT_PROTOCOL_MATCH_BEGIN = new Regex("[-_./]$");

        internal static readonly Regex VALID_CASHTAG = new Regex("(^|" + UNICODE_SPACES + ")(" + DOLLAR_SIGN_CHAR + ")(" + CASHTAG + ")" + "(?=$|\\s|[" + PUNCT_CHARS + "])", RegexOptions.IgnoreCase);
