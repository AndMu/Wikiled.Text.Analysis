...
namespace Wikiled.Text.Analysis.Tokenizer
{
    ...
    public class TreebankWordTokenizer : ITreebankWordTokenizer
    {
        ...
        {
            startingQuotes = new List<ValueTuple<Regex, string>>
                             {
                                 (GetRegex("^\\\""), "``"),
                                 (GetRegex("^''"), @"``"),
                                 (GetRegex("(``)"), @" $1 "),
            ...
            punctuation = new List<ValueTuple<Regex, string>>
                          {
                              (GetRegex(@"([:,])([^\d])"), " $1 $2"),
                              (GetRegex(@"([:,])$"), " $1 "),
                              (GetRegex(@"\.\.\."), " ... "),
            ...
            parensBrackets = new List<ValueTuple<Regex, string>>
                             {
                                 (GetRegex(@"[\]\[\(\)\{\}\<\>]"), " ${0} "),
                                 (GetRegex(@"--"), " -- ")
                             };

            endingQuotes = new List<ValueTuple<Regex, string>>
                            {
                                (GetRegex("\""), " '' "),
                                (GetRegex(@"(\S)(\'\')"), "$1 $2 "),
